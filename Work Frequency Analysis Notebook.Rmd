...
alerts.all.tidy %>%
  ...
  mutate(n_rows = num_rows,
         proportion = n/n_rows) %>%
  top_n(10,wt = n) %>%
  ggplot(aes(x = reorder(word, proportion), label = proportion, y = proportion)) + 
...
```{r echo=FALSE}
alerts.all.tidy %>%
  count(institutionType, word, sort = TRUE) %>%
  top_n(10, wt = n) %>%
  ggplot(aes(x = reorder(word, n), label = n , y = n)) + 
  geom_col(alpha = 0.8, show.legend = FALSE) +
  geom_text(hjust = 0, nudge_y = 0.0005, size = 3) +
  coord_flip() +
  labs(x = "",
       y = "Number of times word appears across all crime alerts") + 
  ggtitle("Most frequent words across all crime alerts") +
  theme(panel.background = element_rect(fill = "transparent", color = NA),
        plot.background = element_rect(fill = "transparent", color = NA))
```
## Public vs. Private {.tabset}

### Proportions

```{r echo=FALSE}
num_rows.private <- nrow(alerts.all.tidy %>% filter(institutionType == "Private"))
num_rows.public <- nrow(alerts.all.tidy %>% filter(institutionType == "Public"))

alerts.all.tidy %>%
  count(institutionType, word, sort = TRUE) %>%
  mutate(n_rows = ifelse(),
         proportion = n/n_rows) %>%
  top_n(10,wt = n) %>%
  ggplot(aes(x = reorder(word, proportion), label = proportion, y = proportion)) + 
  geom_col(alpha = 0.8, show.legend = FALSE) +
